...
func (n *NodeManager) SyncMultiPool(node *Node) error {
	...
	// traverse all the enis, if node doesn't have this pool label, we recycle it.
	if node.resource != nil && node.resource.Status.OpenStack.ENIs != nil {
		for _, eni := range node.resource.Status.OpenStack.ENIs {
			if len(eni.Pool) == 0 {
				continue
			}
			if node.pools[Pool(eni.Pool)] == nil {
